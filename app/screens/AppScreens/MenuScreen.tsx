...
const MenuHeader: FC<MenuHeaderProps> = React.memo(({ onGoBack, onNavigateToSettings }) => {
  const { themed, theme } = useAppTheme();
  return (
    <View style={$headerStyle}>
      <TouchableOpacity style={themed($userIconContainer)} onPress={onGoBack}>
        <Feather name="x" size={24} color={theme.colors.text} />
      </TouchableOpacity>
      <TouchableOpacity style={themed($userIconContainer)} onPress={onNavigateToSettings}>
...
export const MenuScreen: React.FC<MenuScreenProps> = ({ navigation }) => {
   ...
   */
  const handleLongPressRoom = useCallback((room: { id: number; name: string }) => {
  ...
  const renderSkeletonItem = useCallback(
    () => (
      ...
      >
        <SkeletonPlaceholder.Item flexDirection="row" alignItems="center" marginBottom={30}>
          <SkeletonPlaceholder.Item width={40} height={40} borderRadius={10} />
          <SkeletonPlaceholder.Item width={200} height={20} marginLeft={10} borderRadius={5} />
        </SkeletonPlaceholder.Item>
      </SkeletonPlaceholder>
    ),
    [theme.colors.secondaryBg, theme.colors.surface]
  );

  const renderChatItem = useCallback(
    ({ item: chatRoom }: { item: { id: number; name:string } }) => {
      ...
      return (
        <View style={isSelected ? themed($selectedChatContainerStyle) : null}>
          <TouchableOpacity
            ...
            onPress={() => handleSelectRoom(chatRoom.id)}
      ...
      >
        {isLoading ? (
          <Animated.FlatList
            ...
            ListHeaderComponent={
              <MenuHeader onGoBack={handleGoBack} onNavigateToSettings={handleNavigateToSettings} />
            }
          />
        ) : (
          <Animated.FlatList
